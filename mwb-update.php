...
if (! class_exists('Mwb_membership_for_woocommerce_update') ) {
	class Mwb_membership_for_woocommerce_update
	{
		...
		public function mwb_mfw_details()
		{
			...
			if ($tab == 'plugin-information' && $_REQUEST['plugin'] == 'membership-for-woocommerce' ) {
		...
		public function create_html_data( $all_data ) {
			...
			<div class="mwb_mfw_details_wrapper">
				...
				<div class="mwb_mfw_description">
					<h4><?php _e( 'Plugin Description', 'membership-for-woocommerce' ); ?></h4>
					<span><?php echo $all_data['sections']['description']; ?></span>
				</div>
				<div class="mwb_mfw_changelog_details">
					<h4><?php _e( 'Plugin Change Log', 'membership-for-woocommerce' ); ?></h4>
		...
		public function mwb_mfw_in_plugin_update_notice()
		{
			...
			if (isset($data['body']) ) {
				...
				if (is_array($all_data) && ! empty($all_data['sections']['update_notice']) ) {
					...
					echo '</p><div id="mwb_mfw_in_plugin_update_div">' . $all_data['sections']['update_notice'] . '</div><p class="dummy">';
				}
			}
		}
	...
	new Mwb_membership_for_woocommerce_update();
}