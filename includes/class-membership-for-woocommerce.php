...
class Membership_For_Woocommerce {
	...
	private function define_public_hooks() {
		...
		if ( 'on' === $mwb_membership_enable_plugin ) {
			...
			// AJAX handlers to save tnx data.
			$this->loader->add_action( 'wp_ajax_mwb_membership_checkout', $plugin_public, 'mwb_membership_checkout' );
			$this->loader->add_action( 'wp_ajax_nopriv_mwb_membership_checkout', $plugin_public, 'mwb_membership_checkout' );

			// Settin membership price in cart.
			$this->loader->add_action( 'woocommerce_before_calculate_totals', $plugin_public, 'set_membership_product_price' );

			// Settin membership price in cart.
			$this->loader->add_action( 'woocommerce_is_purchasable', $plugin_public, 'make_membership_product_purchasable', 10, 2 );

			// Settin membership price in cart.
			$this->loader->add_action( 'woocommerce_order_status_changed', $plugin_public, 'membership_process_payment' );
