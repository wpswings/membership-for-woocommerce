...
class Membership_For_Woocommerce_Admin {
	...
	public function mfw_admin_enqueue_styles( $hook ) {
		...
		if ( isset( $screen->id ) && ( 'wp-swings_page_home' === $screen->id || 'wp-swings_page_membership_for_woocommerce_menu' === $screen->id ) ) {
			...
			wp_enqueue_style( $this->plugin_name . '-admin-global', MEMBERSHIP_FOR_WOOCOMMERCE_DIR_URL . 'admin/css/membership-for-woocommerce-admin-global.css', array( 'wps-mfw-meterial-icons-css' ), MEMBERSHIP_FOR_WOOCOMMERCE_VERSION, 'all' );
				...
				wp_enqueue_style( 'membership-for-woocommerce-admin-redesign', MEMBERSHIP_FOR_WOOCOMMERCE_DIR_URL . 'admin/css/membership-for-woocommerce-redesign.css', array(), MEMBERSHIP_FOR_WOOCOMMERCE_VERSION, 'all' );
				wp_enqueue_style(
					'membership-for-woocommerce-talk-to-expert',
					MEMBERSHIP_FOR_WOOCOMMERCE_DIR_URL . 'admin/css/membership-for-woocommerce-talk-to-expert.css',
					array( 'membership-for-woocommerce-admin-redesign' ),
					MEMBERSHIP_FOR_WOOCOMMERCE_VERSION,
					'all'
				);
			}
			...
			wp_register_script( $this->plugin_name . 'admin-js', MEMBERSHIP_FOR_WOOCOMMERCE_DIR_URL . 'admin/js/membership-for-woocommerce-admin.js', array( 'jquery', 'wps-mfw-select2', 'wps-mfw-metarial-js', 'wps-mfw-metarial-js2', 'wps-mfw-metarial-lite' ), MEMBERSHIP_FOR_WOOCOMMERCE_VERSION, false );
				wp_localize_script(
					...
					array(
						...
						'mfw_expert_action' => Membership_For_Woocommerce_Talk_To_Expert_Form::AJAX_ACTION,
						'mfw_expert_nonce' => wp_create_nonce( Membership_For_Woocommerce_Talk_To_Expert_Form::NONCE_ACTION ),
					)
				);
				...
				if ( wps_mfw_standard_check_multistep() ) {
					wp_enqueue_script(
						'membership-for-woocommerce-talk-to-expert',
						MEMBERSHIP_FOR_WOOCOMMERCE_DIR_URL . 'admin/js/membership-for-woocommerce-talk-to-expert.js',
		...
		if ( isset( $screen->id ) || isset( $screen->post_type ) ) {
			...
			if ( 'wps_cpt_membership' === $pagescreen_post || 'wps_cpt_membership' === $pagescreen_id || 'wp-swings_page_membership_for_woocommerce_menu' === $screen->id ) {
				...
				wp_localize_script(
					...
					array(
						'ajax_url'  => admin_url( 'admin-ajax.php' ),
						'wps_nonce' => wp_create_nonce( 'wps-offer-nonce' ),
					)
				);
	...
	public function wps_sanitize_array( $wps_input_array ) {
		...
		if ( ! empty( $wps_input_array ) && is_array( $wps_input_array ) ) {
		...
		switch ( $column ) {
			...
			case 'membership_cost':
				...
				if ( ! empty( $currency ) && ! empty( $plan_cost ) ) {
					...
					printf( ' %s %s ', esc_html( $currency ), esc_html( $plan_cost ) );
				}
				break;
		}
	}
	...
	public function wps_membership_for_woo_fill_columns_members( $column, $post_id ) {
		switch ( $column ) {
			...
			case 'membership_id':
				...
				?>
				<strong class="wps_hide_<?php echo esc_attr( $withdrawal_status ); ?>" bulk-user-id="<?php echo esc_html( $author_id ); ?>"><?php printf( ' #%u %s ', esc_html( $post_id ), esc_html( $display_name ) ); ?></strong>
				<?php
				break;
		...
		if ( isset( $_GET['export_all_members'] ) ) {
			...
			if ( ! empty( $all_posts ) ) {
				...
				foreach ( $all_posts as $post_datas ) {
					...
					if ( 'Lifetime' == $this->global_class->get_member_details( $post_datas, 'expiration' ) ) {

						$expiry = $this->global_class->get_member_details( $post_datas, 'expiration' );
					} else {

						$expiry = gmdate( 'Y-m-d', $this->global_class->get_member_details( $post_datas, 'expiration' ) );
					}
					...
					fputcsv(
						...
						array(
							$post_datas->ID,
							get_the_author_meta( 'display_name', wps_membership_get_meta_data( $post_datas->ID, 'wps_member_user', true ) ),
	...
	public function wps_membership_for_woo_save_fields( $post_id ) {
		...
		if ( ! empty( $this->get_plans_default_value() ) && is_array( $this->get_plans_default_value() ) ) {
			...
			if ( ! empty( $offered_product ) && is_array( $offered_product ) ) {
	...
	public function mfw_attach_plan_product_data_tab( $product_data_tabs ) {
		...
		if ( ! empty( $terms_post ) && is_array( $terms_post ) ) {
		...
		if ( ! empty( $product_cat_ids ) && is_array( $product_cat_ids ) ) {
	...
	/**
	 * This function is used to create whatsapp api notification settings.
	 *
	 * @param  array $wps_whatsapp_settings wps_whatsapp_settings.
	 * @return array
	 */
	public function wps_mfw_whatsapp_api_settings( $wps_whatsapp_settings ) {
		...
		$wps_app_settings = array(
			array(
				...
				'type'        => 'radio-switch',
				'description' => __( 'Enable this settings to use whatsapp notification features.', 'membership-for-woocommerce' ),
				'id'          => 'wps_wpr_enable_whatsapp_api_feature',
				'value'       => get_option( 'wps_wpr_enable_whatsapp_api_feature' ),
				'class'       => 'mfw-radio-switch-class',
				'options'     => array(
					'yes' => __( 'YES', 'membership-for-woocommerce' ),
					'no'  => __( 'NO', 'membership-for-woocommerce' ),
	...
	/**
	 * Save Whatsapp API settings here.
	 *
	 * @return void
	 */
	public function mfw_admin_save_whatsapp_api_settings() {
	...
	public function wps_wpr_send_offer_message_via_sms() {
		...
		if ( $success_count > 0 ) {
			...
			$response = array(
				'result' => true,
				'msg' => esc_html__( 'Your offer messages has been sent successfully!', 'membership-for-woocommerce' ),
			);
