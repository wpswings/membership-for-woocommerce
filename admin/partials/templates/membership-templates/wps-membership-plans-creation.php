					...
					if ( function_exists( 'check_membership_pro_plugin_is_active' ) ) {
						...
						if ( $check_licence ) {
							if ( ! empty( $settings_fields ) ) {
								...
								if ( ! empty( $wps_membership_club ) && is_array( $wps_membership_club ) ) {
						...
						if ( function_exists( 'check_membership_pro_plugin_is_active' ) ) {
							...
							if ( $check_licence ) {
								if ( ! empty( $settings_fields ) ) {
									...
									if ( ! empty( $wps_membership_plan_target_tags ) && is_array( $wps_membership_plan_target_tags ) ) {
										foreach ( $wps_membership_plan_target_tags as $single_target_tag_id ) {

											$tagn = get_term_by( 'id', $single_target_tag_id, 'product_tag' );
											?>
											<option value="<?php echo esc_html( $single_target_tag_id ); ?>" <?php selected( in_array( $single_target_tag_id, $wps_membership_plan_target_tags, true ) ); ?>><?php echo( esc_html( $tagn->name ) . '(#' . esc_html( $single_target_tag_id ) . ')' ); ?></option>
											<?php
										}
									}
								}
			...
			<tr class="wps-membership__plan--pro-disabled">
				...
				<td class="forminp forminp-text">
					...
					<select name="wps_membership_product_offer_price_type" id='wps_membership_product_offer_price_type_id' >
						<option <?php selected( $wps_membership_product_offer_price_type, '%' ); ?> value="%"><?php esc_html_e( 'Discount %', 'membership-for-woocommerce' ); ?></option>
						<option <?php selected( $wps_membership_product_offer_price_type, 'fixed' ); ?> value="fixed"><?php esc_html_e( 'Fixed price', 'membership-for-woocommerce' ); ?></option>
					</select>
		...
		<div class="new_created_offers wps_membership_offers" id="new_created_offers" >
						...
						if ( function_exists( 'check_membership_pro_plugin_is_active' ) ) {
							...
							if ( $check_licence ) {
								if ( ! empty( $settings_fields ) ) {
									...
									if ( ! empty( $wps_membership_plan_target_page_ids ) && is_array( $wps_membership_plan_target_page_ids ) ) {
										foreach ( $wps_membership_plan_target_page_ids as $wps_membership_plan_single_target_page_ids ) {
											...
											?>
											<option value="<?php echo esc_html( $wps_membership_plan_single_target_page_ids ); ?>" <?php selected( in_array( $wps_membership_plan_single_target_page_ids, $wps_membership_plan_target_page_ids, true ) ); ?>><?php echo( esc_html( $page_name ) . '(#' . esc_html( $wps_membership_plan_single_target_page_ids ) . ')' ); ?></option>
											<?php
										}
									}
								}
						...
						if ( function_exists( 'check_membership_pro_plugin_is_active' ) ) {
							...
							if ( $check_licence ) {
								if ( ! empty( $settings_fields ) ) {
									...
									if ( ! empty( $wps_membership_plan_target_post_ids ) && is_array( $wps_membership_plan_target_post_ids ) ) {
										foreach ( $wps_membership_plan_target_post_ids as $wps_membership_plan_single_target_post_ids ) {
											...
											?>
											<option value="<?php echo esc_html( $wps_membership_plan_single_target_post_ids ); ?>" <?php selected( in_array( $wps_membership_plan_single_target_post_ids, $wps_membership_plan_target_post_ids, true ) ); ?>><?php echo( esc_html( $post_name ) . '(#' . esc_html( $wps_membership_plan_single_target_post_ids ) . ')' ); ?></option>
											<?php
										}
									}
								}
						...
						if ( function_exists( 'check_membership_pro_plugin_is_active' ) ) {
							...
							if ( $check_licence ) {
								if ( ! empty( $settings_fields ) ) {
									...
									if ( ! empty( $wps_membership_plan_target_post_categories ) && is_array( $wps_membership_plan_target_post_categories ) ) {
										foreach ( $wps_membership_plan_target_post_categories as $single_target_category_id ) {

											$category_name = $instance->get_category_title( $single_target_category_id );
											?>
											<option value="<?php echo esc_html( $single_target_category_id ); ?>" <?php selected( in_array( $single_target_category_id, $wps_membership_plan_target_post_categories, true ) ); ?>><?php echo( esc_html( $category_name ) . '(#' . esc_html( $single_target_category_id ) . ')' ); ?></option>
											<?php
										}
									}
								}
						...
						if ( function_exists( 'check_membership_pro_plugin_is_active' ) ) {
							...
							if ( $check_licence ) {
								if ( ! empty( $settings_fields ) ) {
									...
									if ( ! empty( $wps_membership_plan_target_post_tags ) && is_array( $wps_membership_plan_target_post_tags ) ) {

										foreach ( $wps_membership_plan_target_post_tags as $single_target_tag_id ) {
											$tagn = get_term_by( 'id', $single_target_tag_id, 'post_tag' );
											?>
											<option value="<?php echo esc_html( $single_target_tag_id ); ?>" selected="selected"><?php echo( esc_html( $tagn->name ) . '(#' . esc_html( $single_target_tag_id ) . ')' ); ?></option>
											<?php
										}
									}
								}
			...
			<tr id="mfw_membership_access_type" class="wps-membership__plan--pro-disabled">
				...
				<td id="mfw_offer_access_type">
					...
					?>
					<input type="radio" id="wps_membership_plan_immediate_type" name="wps_membership_plan_access_type" value="immediate_type" <?php checked( $wps_membership_plan_access_type, 'immediate_type', true ); ?>>
					<label for="wps_membership_plan_immediate_type"><?php esc_html_e( 'Immediately', 'membership-for-woocommerce' ); ?></label>
					<input type="radio" id="wps_membership_plan_time_type" name="wps_membership_plan_access_type" value="delay_type" <?php checked( $wps_membership_plan_access_type, 'delay_type', true ); ?>>
					...
					<div id="wps_membership_plan_time_duratin_display">
						...
						<select name="wps_membership_plan_time_duration_type" id="wps_membership_plan_time_duration_type" >
							<option <?php selected( $wps_membership_plan_time_duration_type , 'days'); ?> value="days"><?php esc_html_e( 'Days', 'membership-for-woocommerce' ); ?></option>
							<option <?php selected( $wps_membership_plan_time_duration_type, 'weeks' ); ?> value="weeks"><?php esc_html_e( 'Weeks', 'membership-for-woocommerce' ); ?></option>
						</select>
					</div>
				</td>
			</tr>
