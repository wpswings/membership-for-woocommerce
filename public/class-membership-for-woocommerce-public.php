...
class Membership_For_Woocommerce_Public {
	...
	public function wps_membership_process_payment( $order_id ) {
		...
		$this->wps_mfw_send__welcome_mail( $order_id );
	}
	...
	/**
	 * This function is used to send a welcome email when a membership
	 * is purchased for the first time.
	 *
	 * @param  int $order_id order_id.
	 * @return void
	 */
	public function wps_mfw_send__welcome_mail( $order_id ) {

		$order = wc_get_order( $order_id );
		if ( 'completed' == $order->get_status() ) {

			$wps_mfw_welcome_mail_sended = get_post_meta( $order_id, 'wps_mfw_welcome_mail_sended', true );
						...
						if ( empty( $wps_mfw_welcome_mail_sended ) ) {
							...
							if ( ! empty( $current_memberships[0] ) && is_array( $current_memberships ) ) {
								...
								if ( ! empty( $memberships_plan_obj ) && is_array( $memberships_plan_obj ) ) {
									...
									update_post_meta( $order_id, 'wps_mfw_welcome_mail_sended', 'done' );
								}
							}
						}
					}
